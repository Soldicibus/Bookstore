...
class BookService {
  ...
  static async getBooksByGenre(genreId, db = pool) {
    try {
      const books = await BookModel.findByGenre(genreId, db);
      return { books };
    } catch (error) {
      console.error("Service Error in getBooksByGenre:", error.message);
      throw error;
    }
  }

  static async getBooksByAuthor(authorId, db = pool) {
    try {
      const books = await BookModel.findByAuthor(authorId, db);
      return { books };
    } catch (error) {
      console.error("Service Error in getBooksByAuthor:", error.message);
      throw error;
    }
  }

  static async getBooksByThematic(thematicId, db = pool) {
    try {
      const books = await BookModel.findByThematic(thematicId, db);
      return { books };
    } catch (error) {
      console.error("Service Error in getBooksByThematic:", error.message);
      throw error;
    }
  }

  static async getAuthorsByBook(bookId, db = pool) {
    try {
      const authors = await BookModel.findAuthorsByBook(bookId, db);
      return { authors };
    } catch (error) {
      console.error("Service Error in getAuthorsByBook:", error.message);
      throw error;
    }
  }
  ...
  static async addToFavorites(userId, bookId, db = pool) {
    try {
      const res = await BookModel.addToFavorites(userId, bookId, db);
      return { res, message: "Book added to favorites" };
    } catch (error) {
      console.error("Service Error in addToFavorites:", error.message);
      throw error;
    }
  }
  ...
  static async removeFromFavorites(userId, bookId, db = pool) {
    try {
      const res = await BookModel.removeFromFavorites(userId, bookId, db);
      return { res, message: "Book removed from favorites" };
    } catch (error) {
      console.error("Service Error in removeFromFavorites:", error.message);
      throw error;
    }
  }

  static async getFavoriteBooks(userId, db = pool) {
	try {
	  const books = await BookModel.findFavoritesByUser(userId, db);
	  return { books };
	} catch (error) {
	  console.error("Service Error in getFavoriteBooks:", error.message);
	  throw error;
	}
  }
}
