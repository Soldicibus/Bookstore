...
export function useRemoveThematicFromBook() {
  const qc = useQueryClient();
  return useMutation(({ bookId, thematicId }) => bookAPI.removeThematicFromBook(bookId, thematicId), {
    onMutate: async ({ bookId, thematicId }) => {
      ...
      qc.setQueryData(["book", bookId], (old) => {
        if (!old) return old;
        const thematics = old.thematics ? old.thematics.filter((t) => t.id !== thematicId) : [];
